...
def do():
    ...
    if var.get()==1:
        ...
        ans=acmakeGiopenjtalk.acmake(word)
        lab12["text"] = str(ans[0:2])
        EUAT2HTS.HTS(ans[0],ans[1],ans[3])
        
    elif var.get()==2:
        st()
        lab12.configure(state='active')
        ans=acmakephounidic.acmake(word,strong)
        lab12["text"] = str(ans) 
        EUAT2HTS.HTS(ans[0],ans[1],strong)
        
    elif var.get()==3:
        st()
        lab12.configure(state='active')
        ans=acmakesurunidic.acmake(word,strong)
        lab12["text"] = str(ans)
        EUAT2HTS.HTS(ans[0],ans[1],strong)
        ...
        st()
        lab12.configure(state='active')
        ans=acmakeposunidic.acmake(word,strong)
        lab12["text"] = str(ans)
        EUAT2HTS.HTS(ans[0],ans[1],strong)
...
#widget
lab1 = tk.Label(Frame1, text="複合語(textエディタでローマ字入力した文字のコピペなら確実)")
...
lab7.configure(state='disabled')
lab15=tk.Radiobutton(Frame4,variable=var,value=4,text="文法考慮CRF",command=touch)
lab15.configure(state='disabled')
